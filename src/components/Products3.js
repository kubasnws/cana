...
class Products3 extends Component {
  ...
  onScroll = (e) => {
    ...
    if (e.deltaY < 0 && !debounce) {
      ...
      //Down
      localization === "news"
        ? this.props.history.push(routes.newsFooter)
        : this.props.history.push(routes.productsFooter);
    }
  };
  ...
  render() {
    ...
    const prodPageApi =
      this.props.productsPageData && this.props.productsPageData[0];
    const { acf: { top_image_2: topImage } = Object } = prodPageApi
      ? prodPageApi
      : Object;
    const insta = this.props.insta;

    const generateElement =
      insta && insta.map((item) => <InstaElement key={item.id} data={item} />);
...
const InstaElement = ({
  data: {
    link,
    created_time: time,
    caption: { text: description } = null,
    images: {
      standard_resolution: { url: image },
    },
  },
}) => {
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Products3)
);
