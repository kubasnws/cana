    ...
    return (
      <div className={s.mainBox}>
        <div className={s.topBanner}>
          ...
          {acf && (
            <img
              src={acf[0].acf.top_image.url}
              alt={acf[0].acf.top_image.mime_type}
            />
          )}
        </div>
        ...
        <div className={s.bottomButtons}>
          <div className={[s.button, s.leftButton].join(" ")}>
            <DelayLink to={routes.productsHome}>
              <div>
                <ChevronUp />
                <span>
                  {lang === "en"
          ...
          <div className={[s.button, s.rightButton].join(" ")}>
            ...
            >
              <div>
                <span>Catch the dark horse</span>
                <ChevronDown />
              </div>
            </DelayLink>
          </div>
...
const SwiperElements = ({ products = [] }) => {
  const result = products.map(
    (item) => products.length > 0 && <SwiperElement key={item.id} item={item} />
  );
  return <>{result}</>;
};
...
const SwiperElement = (props) => {
  ...
  return (
    <div
      className={[s.swiperSlide, "swiper-slide"].join(" ")}
      data-hash={props.item.id}
    >
      <img src={url} alt={name} />
      <div className={s.productDescription}>{short_description}</div>
    </div>
  );
};
